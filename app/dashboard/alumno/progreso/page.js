          ...
          {ultimo && (
            <div className="text-2xl font-black text-foreground">
              ...
              {registros.length > 1 && (() => {
                ...
                const color = diff < 0 ? 'text-green-500' : diff > 0 ? 'text-pink-400' : 'text-zinc-500'
                return (
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-1.5 sm:gap-2">
            <input
              ...
              placeholder="kg"
              className="w-16 sm:w-24 min-w-0 bg-raised border border-border text-foreground rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-pink-600 transition-colors"
            />
            <DateInput
              value={fecha}
              onChange={e => setFecha(e.target.value)}
              wrapperClassName="relative w-36 sm:w-40"
            ...
            <div key={r.id} className="flex items-center justify-between text-xs">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => eliminar(r.id)}
                  className="text-zinc-600 hover:text-pink-400 transition-colors">✕</button>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="max-w-5xl space-y-5">
      ...
      <div>
        ...
        <p className="text-xs text-zinc-500 mt-1">Tu historial de entrenamiento en Pinkline</p>
      </div>
