...
export const crearCuentaAlumnoSchema = z.object({
  email:  z.string().email('Email inválido'),
  rut:    z.string().max(20).optional().nullable(),
...
// ── Pagos de alumnas (contabilidad) ───────────────────────────────────────────

export const pagoAlumnoSchema = z.object({
  alumno_id:     uuid,
  fecha_pago:    fecha,
  meses_pagados: z.number().int().min(1).max(12),
  monto:         z.number().int().min(0).nullable().optional(),
  notas:         z.string().max(500).optional().nullable(),
})

export const actualizarPagoAlumnoSchema = pagoAlumnoSchema.partial()
